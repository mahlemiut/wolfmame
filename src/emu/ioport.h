...
public:
	...
	{
		return get_string<OFFS_APPDESC, OFFS_ENDTIME>();
	}
	...
	void set_endtime(std::uint64_t time)
	{
		m_data[OFFS_ENDTIME + 0] = std::uint8_t((time >> (0 * 8)) & 0x00ff);
		m_data[OFFS_ENDTIME + 1] = std::uint8_t((time >> (1 * 8)) & 0x00ff);
		m_data[OFFS_ENDTIME + 2] = std::uint8_t((time >> (2 * 8)) & 0x00ff);
		m_data[OFFS_ENDTIME + 3] = std::uint8_t((time >> (3 * 8)) & 0x00ff);
		m_data[OFFS_ENDTIME + 4] = std::uint8_t((time >> (4 * 8)) & 0x00ff);
		m_data[OFFS_ENDTIME + 5] = std::uint8_t((time >> (5 * 8)) & 0x00ff);
		m_data[OFFS_ENDTIME + 6] = std::uint8_t((time >> (6 * 8)) & 0x00ff);
		m_data[OFFS_ENDTIME + 7] = std::uint8_t((time >> (7 * 8)) & 0x00ff);
	}
	void set_version()
	{
	...
	{
		set_string<OFFS_APPDESC, OFFS_ENDTIME>(desc);
	}
...
private:
	...
	static constexpr std::size_t    OFFS_ENDTIME     = 0x38;    // 0x38 bytes (little-endian binary integer)
