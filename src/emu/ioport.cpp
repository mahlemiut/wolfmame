...
void ioport_manager::record_init()
{
	...
	header.set_appdesc(util::string_format("%s", emulator_info::get_build_version()));
...
bool ioport_manager::get_record_file()
{
	if (m_record_file)
		return m_record_file->is_open();
	else
		return false;
}

bool ioport_manager::get_playback_file()
{
	if(m_playback_file)
		return m_playback_file->is_open();
	else
		return false;
}
