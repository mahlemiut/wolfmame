...
uint32_t mame_ui_manager::handler_ingame()
{
	...
	if (inp.pressed(IPT_UI_SAVE_STATE) && !machine().ioport().get_record_file())
	{
		m_ui_target = &current_ui_target();
	...
	if (inp.pressed(IPT_UI_LOAD_STATE) && !machine().ioport().get_record_file())
	{
		m_ui_target = &current_ui_target();
	...
	// toggle pause
	if (inp.pressed(IPT_UI_PAUSE) && !machine().ioport().get_record_file())
	...
	// rewind single step
	if (inp.pressed(IPT_UI_REWIND_SINGLE) && !machine().ioport().get_record_file())
		machine().rewind_step();

	// handle a toggle cheats request
	if (inp.pressed(IPT_UI_TOGGLE_CHEAT) && !machine().ioport().get_record_file())
	...
	// toggle throttle
	if (inp.pressed(IPT_UI_THROTTLE) && !machine().ioport().get_record_file())
